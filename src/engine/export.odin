...
// The compiler's output stage (DESIGN.md §1): strata is at its core an
// svg → obj compiler, and this file is the "→ obj" half. Everything a
// headless consumer needs — the CLI, golden tests, a game importer, CI —
// lives here, never in the frontend:
//
//   mesh_checksum          golden-test primitive: FNV over quantized verts
//   mesh_obj_write/save    OBJ tri-soup, `usemtl <recipe-name>` per run
//   level_meta_write/save  game-agnostic level sidecar (.txt) — everything
//                          the OBJ cannot carry: document meta lines,
//                          material table, markers (heights resolved via
//                          surface_height, the walk-surface query), portals
//
// The sidecar format is line-oriented, space-separated, game-agnostic:
//   level    <name>
...
// `args…` runs to end of line. y is up; (x, z) = document (x, y), matching
// the OBJ. A marker over raw rock (no surface) grounds at y=0.
...
// mesh_checksum — FNV-1a over quantized vertex data and triangle counts, so
// golden tests (DESIGN.md §5) catch geometry drift without storing meshes.
...
// mesh_obj_write — the tri-soup as OBJ: positions + normals, one `g` per
// chunk, `usemtl <recipe-name>` runs from the face's first vertex.
mesh_obj_write :: proc(w: ^Eval_World, chunks: []Mesh_Chunk, b: ^strings.Builder, note: string) {
	...
	for &c in chunks {
		...
		for i := 0; i < len(c.indices); i += 3 {
			...
			if mat != cur_mat && int(mat) < len(w.materials) {
				fmt.sbprintfln(b, "usemtl %s", name32_str(&w.materials[mat]))
				cur_mat = mat
			}
...
mesh_obj_save :: proc(w: ^Eval_World, chunks: []Mesh_Chunk, path: string, note: string) -> bool {
	b := strings.builder_make(context.temp_allocator)
	mesh_obj_write(w, chunks, &b, note)
...
@(private = "file")
dash_name :: proc(n: ^Name32) -> string {
	s := name32_str(n)
	if s == "" {return "-"}
...
level_meta_write :: proc(doc: ^Document, w: ^Eval_World, b: ^strings.Builder) {
	...
	for &m, i in w.materials {
		fmt.sbprintfln(b, "material %d %s", i, name32_str(&m))
	}
	for &c in doc.components {
		if c.kind == .Marker && len(c.points) > 0 {
			pos := c.points[0].pos
			y, _ := surface_height(w, pos)
			args := c.args
		...
		for &t in c.edge_tags {
			...
			ya, _ := surface_height(w, a)
			yb, _ := surface_height(w, pb)
			tag := t
			fmt.sbprintf(b, "portal %s %d %.5f %.5f %.5f %.5f %.5f %.5f",
				dash_name(&c.name), t.segment, a.x, ya, a.y, pb.x, yb, pb.y)
			if args := name32_str(&tag.args); args != "" {fmt.sbprintf(b, " %s", args)}
			fmt.sbprintln(b)
		}
	}
}

level_meta_save :: proc(doc: ^Document, w: ^Eval_World, path: string) -> bool {
	b := strings.builder_make(context.temp_allocator)
